...
from bot.state.dialog_state import CompanyAdminAddEditUserSG
from bot.dialogs.company_administration.add_edit_user.handler import (btn_next_or_end, error_login,
                                                                      btn_next_or_end_login, btn_delete,
                                                                      error_password, btn_confirm, btn_back)
from bot.dialogs.company_administration.add_edit_user.getter import input_getter, card_user_getter
...
input_login_window = Window(
    ...
    CANCEL_EDIT,
    state=CompanyAdminAddEditUserSG.INPUT_LOGIN,
    getter=input_getter
)
...
input_password_window = Window(
    ...
    CANCEL_EDIT,
    state=CompanyAdminAddEditUserSG.INPUT_PASSWORD,
    getter=input_getter
)
...
company_admin_add_edit_user_dialog = Dialog(
    input_username_window, input_login_window, input_password_window, card_user_window
)

company_admin_add_edit_user_dialog.message.middleware(AuthorizationMiddleware())
company_admin_add_edit_user_dialog.callback_query.middleware(AuthorizationMiddleware())
